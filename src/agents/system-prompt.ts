...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    desktop: "Control the local computer's mouse/keyboard and take screenshots (Computer Use)",
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- canvas: present/eval/snapshot the Canvas",
          "- desktop: control local mouse/keyboard (Computer Use)",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "You have access to a `desktop` tool that allows you to control the local computer's mouse and keyboard.",
    "- **When to use**: Use `desktop` for OS-level interactions that `browser` cannot handle (e.g., system settings, native apps, Finder, menu bar).",
    "- **Strategy**: Always start by checking screen size (`screen_size`) and taking a screenshot (`screenshot`) to understand the UI layout before moving the mouse.",
    "- **Loop**: Observe (screenshot) -> Think (coordinates) -> Act (mouse/keyboard). Repeat as needed.",
    "- **Safety**: Be careful with clicks. Double-check coordinates from the screenshot.",
    "",
    "## Tool Call Style",
