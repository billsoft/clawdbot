...
export const SECTION_META: Record<string, { label: string; description: string }> = {
  ...
  update: { label: "更新", description: "自动更新设置和发布频道" },
  ...
  commands: { label: "指令", description: "自定义斜杠指令" },
  ...
  talk: { label: "语音", description: "语音和对话设置" },
...
export function renderConfigForm(props: ConfigFormProps) {
  if (!props.schema) {
    return html`<div class="muted">架构不可用。</div>`;
  }
  const schema = props.schema;
  const value = props.value ?? {};
  if (schemaType(schema) !== "object" || !schema.properties) {
    return html`<div class="callout danger">不支持的架构。请使用原始模式。</div>`;
  }
